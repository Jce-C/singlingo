...
const HOLD_REQUIRED     = 18;   // frames consecutivos para confirmar letra (~0.6s)
...
class SpellMode {
  ...
  _bindUI() {
    document.addEventListener("handsUpdate", (e) => {
      ...
      const { right, left } = e.detail;
      const dominant = right ?? left;
      if (!dominant) return;

      this._drawLiveHand(dominant.landmarks, dominant.worldLandmarks ?? null);
    ...
    document.getElementById("btn-spell-skip")
      ?.addEventListener("click", () => this._nextWord());

    document.getElementById("btn-spell-restart")
      ?.addEventListener("click", () => this.startQuiz());
  }
  ...
  async _predict(landmarksR, landmarksL, worldR, worldL) {
  ...
  _checkLetterMatch(signId, conf) {
    ...
    if (signId === target && conf >= MIN_CONFIDENCE) {
      this.holdFrames++;
  ...
  async _startCamera() {
    ...
    this.mp = new MediaPipeHandler({ numHands: 1, captureMode: false });
  ...
  _drawLiveHand(landmarks, worldLandmarks = null) {
