...
async def ws_predict(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            right_raw = lms_r if (lms_r and len(lms_r) == 21) else (lms if len(lms) == 21 else None)
            left_raw  = lms_l if (lms_l and len(lms_l) == 21) else None
            ...
            if target in DYNAMIC_SIGNS:
                ...
                if motion_phase == "motion":
                    ...
                    TIP_IDX_MAP = {"J": 20, "Z": 8, "S": 0, "Ñ": 12, "G": 8, "H": 12}
                    if m_sign:
...
async def train_status(country: str):
    ...
    for sign in signs_raw:
        ...
        signs_data.append(SignSampleCount(sign_id=sid, count=count, enough=enough))
