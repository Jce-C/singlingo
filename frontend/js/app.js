...
const App = {
  ...
  async _checkHealth() {
    ...
    const allTrained = Object.values(health.model_loaded).every(Boolean);
    if (!allTrained) {
      const banner = document.getElementById("no-model-banner");
      if (banner) {
        banner.textContent = "⚠️ Modelo de IA no entrenado. Ejecuta: python scripts/generate_data.py && python scripts/train_model.py";
  ...
  setCountry(country) {
    ...
    if (this.currentSection === "learn") {
      this.learnMode.setCountry(country);
    } else if (this.currentSection === "practice") {
      this.practiceMode.stop();
      this.practiceMode.start(country);
    } else if (this.currentSection === "spell") {
      this.spellMode.setCountry(country);
    }
  },
  ...
  _setupCountrySelector() {
    document.querySelectorAll(".country-btn").forEach(btn => {
      btn.addEventListener("click", () => this.setCountry(btn.dataset.country));
    });
  ...
  async loadProgressModal() {
    ...
    if (ach) {
      const levels = [
        { condition: summary.total_attempts >= 1,      label: "🏆 Primera seña intentada",    cls: "level-1" },
        { condition: summary.best_streak >= 5,         label: "🔥 Racha de 5 correctas",       cls: "level-2" },
        { condition: summary.signs_mastered >= 10,     label: "⭐ 10 señas dominadas",         cls: "level-3" },
        { condition: summary.signs_mastered >= 26,     label: "🌟 Maestro del Alfabeto",        cls: "level-4" },
      ];
